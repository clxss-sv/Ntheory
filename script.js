(function(){
  ...
  function render(){
    ...
    // อัตราส่วนต้นแบบอ้างอิงที่ 1 ไร่ (RAI_SQM = 1,600 ตร.ม.):
    //   neededTotal  = พื้นที่ 4 ส่วนตามความต้องการจริงของแบบจำลอง (x:y:z:w)
    //   walkwayRef   = พื้นที่ว่างสำหรับเดินเท้า/ทางเดิน ที่เหลือจาก 1 ไร่ (เช่น 1,315 + 285 = 1,600)
    //   referenceTotal = ฐานอ้างอิงของสัดส่วนทั้งหมด (4 ส่วน + ทางเดิน) ไม่ขึ้นกับขนาดที่ดินที่เลือก
    // เมื่อผู้ใช้ปรับขนาดที่ดินทั้งหมด (ไร่) ทุกส่วน "รวมทางเดิน" จะถูกคูณด้วย scaleFactor เดียวกัน
    // เพื่อรักษาสัดส่วนเดิมไว้เสมอ แล้วรวมกันได้พอดีกับขนาดที่ดินที่เลือกไว้เสมอ
    var walkwayRef = Math.max(0, RAI_SQM - neededTotal);
    var referenceTotal = neededTotal + walkwayRef; // = max(RAI_SQM, neededTotal)

    var landShort = landAreaSqm < referenceTotal;
    var scaleFactor = referenceTotal > 0 ? (landAreaSqm / referenceTotal) : 1;
    var walkwayArea = walkwayRef * scaleFactor;
    ...
    if (landCompareEl) {
      var compareHtml;
      if (landShort) {
        var diff = referenceTotal - landAreaSqm;
        compareHtml = '<p class="land-note land-note-warn">ที่ดินที่เลือก ' + fmt(landAreaSqm, 0) + ' ตร.ม. (' + raiLabel + ') น้อยกว่าความต้องการขั้นต่ำของแบบจำลอง (' + fmt(referenceTotal, 0) + ' ตร.ม.) อยู่ ' + fmt(diff, 0) + ' ตร.ม. — พื้นที่แต่ละส่วนด้านล่างจึงถูกย่อลงตามสัดส่วนเดิมให้พอดีกับที่ดินที่มี (ไม่เหลือพื้นที่สำหรับทางเดิน)</p>';
      } else if (walkwayArea > 0.005) {
        compareHtml = '<p class="land-note land-note-ok">ที่ดินที่เลือก ' + fmt(landAreaSqm, 0) + ' ตร.ม. (' + raiLabel + ') — พื้นที่ทั้ง 4 ส่วนขยาย/ย่อตามสัดส่วนเดิมจากฐานอ้างอิง 1 ไร่ (' + fmt(neededTotal, 0) + ' ตร.ม. ต่อทางเดิน ' + fmt(walkwayRef, 0) + ' ตร.ม.) เป็น <strong>' + fmt(neededTotal * scaleFactor, 0) + ' ตร.ม.</strong> สำหรับ 4 ส่วนด้านล่าง และ <strong>' + fmt(walkwayArea, 0) + ' ตร.ม.</strong> เป็นพื้นที่ว่างสำหรับเดินเท้า/ทางเดิน ตามสัดส่วนเดียวกัน</p>';
      } else {
        compareHtml = '<p class="land-note land-note-ok">ที่ดินที่เลือก ' + fmt(landAreaSqm, 0) + ' ตร.ม. (' + raiLabel + ') พอดีกับความต้องการจริงจากแบบจำลอง โดยไม่มีพื้นที่ว่างสำหรับทางเดินเหลือ</p>';
      }
